...
/***/ (function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;// Support for Dark mode
	// Tooltips
	// Tokens
	// Click nodes
	// Zoom to node?
	    ...
	    var vizObj = {
	        ...
	        doDraw: function(){
	                ...
	                for (var j = 0; j < parts.length; j++) {
	                    ...
	                    if (j + 1 < parts.length) {
	                        ...
	                        if (!foundChild) {
	                            ...
	                            childNode = {"name": nodeName, "drilldown": drilldown};
	                            children.push(childNode);
	                        }
	                        ...
	                        // if (viz.data.results[i].hasOwnProperty("size")) {
	                        //     childNode.size = viz.data.results[i].size;
	                        // }
	            ...
	            if (skippedRows && ! validRows) {
	                viz.$container_wrap.empty().append("<div class='dendrogram_viz-bad_data'>Invalid Data, there should be a column called 'path' delimited by forward slash: '/'.</div>");
	                return;
	            }
	            ...
	            //     var crumbs = [$("<div class='dendrogram_viz-first'></div>")];
	            //     var tt = $("<div></div>");
	            //     for (var i = 1; i < parts.length; i++) {
	            //         $("<span></span>").text(parts[i]).appendTo(tt);
	            //         crumbs.push($("<div class='dendrogram_viz-crumb'></div>") /*.css("background-color", color(d.data.name)) */ .text(parts[i]));
	            //         if (i < (parts.length - 1)) {
	            //             $("<span class='dendrogram_viz-tooltip-divider'> / </span>").appendTo(tt);
	            //         }
	            //     }
	            ...
	            var dataInHierarchy = d3.hierarchy(data).sort(function(a, b) {
	                return d3.descending(a.height, b.height);// || d3.ascending(a.data.name, b.data.name)
	            });
	            if (viz.config.tidy === "yes") {
	                if (viz.config.layout !== "radial") {
	                    ...
	                    root = d3.tree()
	                        ...
	                        .separation((a, b) => (a.parent == b.parent ? 1 : 2) / a.depth)(dataInHierarchy);
	                }
	            } else {
	                if (viz.config.layout !== "radial"){
	            ...
	            svg_canvas.append("g")
	                ...
	                .attr("stroke", function(d) { return d.data.hasOwnProperty("color") ? tinycolor(d.data.color).darken(20).toString() : "black"; })
	                .attr("fill", function(d) { return d.data.hasOwnProperty("color") ? d.data.color : d.children ? "#555" : "#999"; })
